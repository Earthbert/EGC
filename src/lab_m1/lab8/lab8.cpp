...
void Lab8::Init() {
	...
	// Light & material properties
	{
		...
		lightDirection[0] = normalize(glm::vec3(0, -1, -1));

		lightPosition[1] = glm::vec3(-1, 1, 2);
		lightDirection[1] = normalize(glm::vec3(0, -1, 1));
...
void Lab8::Update(float deltaTimeSeconds) {
	...
	// Render the point light1 in the scene
	{
		glm::mat4 modelMatrix = glm::mat4(1);
	...
	// Render the point light2 in the scene
	{
		glm::mat4 modelMatrix = glm::mat4(1);
...
void Lab8::RenderSimpleMesh(Mesh* mesh, Shader* shader, const glm::mat4& modelMatrix, const glm::vec3& color) {
	...
	glUniform3f(light_direction, lightDirection[0].x, lightDirection[0].y, lightDirection[0].z);

	// Set shader uniforms for light properties
	light_position = glGetUniformLocation(shader->program, "light_position[1]");
	glUniform3f(light_position, lightPosition[1].x, lightPosition[1].y, lightPosition[1].z);

	light_direction = glGetUniformLocation(shader->program, "light_direction[1]");
	glUniform3f(light_direction, lightDirection[1].x, lightDirection[1].y, lightDirection[1].z);
	...
	int is_spot_light = glGetUniformLocation(shader->program, "is_spot_light[0]");
	glUniform1i(is_spot_light, isSpotLight[0]);

	// TODO(student): Set any other shader uniforms that you need
	is_spot_light = glGetUniformLocation(shader->program, "is_spot_light[1]");
	glUniform1i(is_spot_light, isSpotLight[1]);
...
void Lab8::OnInputUpdate(float deltaTime, int mods) {
	...
	if (!window->MouseHold(GLFW_MOUSE_BUTTON_RIGHT)) {
		...
		if (window->KeyHold(GLFW_KEY_1)) {
			...
			// Control light position using on W, A, S, D, E, Q
			if (window->KeyHold(GLFW_KEY_W)) lightPosition[1] -= forward * deltaTime * speed;
			if (window->KeyHold(GLFW_KEY_A)) lightPosition[1] -= right * deltaTime * speed;
			if (window->KeyHold(GLFW_KEY_S)) lightPosition[1] += forward * deltaTime * speed;
			if (window->KeyHold(GLFW_KEY_D)) lightPosition[1] += right * deltaTime * speed;
			if (window->KeyHold(GLFW_KEY_E)) lightPosition[1] += up * deltaTime * speed;
			if (window->KeyHold(GLFW_KEY_Q)) lightPosition[1] -= up * deltaTime * speed;
...
void Lab8::OnKeyPress(int key, int mods) {
	...
	if (window->KeyHold(GLFW_KEY_1)) {
		if (key == GLFW_KEY_F) {
			isSpotLight[1] = -isSpotLight[1];
		}
	}
	if (window->KeyHold(GLFW_KEY_0)) {
		if (key == GLFW_KEY_F) {
			isSpotLight[0] = -isSpotLight[0];
		}
	}
