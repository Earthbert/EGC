...
void Lab7::Update(float deltaTimeSeconds) {
	...
	// Render ground
	{
		...
		RenderSimpleMesh(meshes["plane"], shaders["LabShader"], modelMatrix, glm::vec3(0.5, 0.4, 0.2));
	}
...
void Lab7::RenderSimpleMesh(Mesh* mesh, Shader* shader, const glm::mat4& modelMatrix, const glm::vec3& color) {
	...
	GLint loc_specular_type = glGetUniformLocation(shader->program, "specular_type");
	glUniform1i(loc_specular_type, specular_type);
