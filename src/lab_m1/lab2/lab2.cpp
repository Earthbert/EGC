...
void Lab2::Init()
{
	...
	// Create a mesh tetrahedron using custom data
	{
		vector<VertexFormat> vertices
		{
			VertexFormat(glm::vec3(sqrt(8.0 / 9.0), 0,  -1.0 / 3.0), glm::vec3(1, 0, 0), glm::vec3(-1, 0, 1)),
			VertexFormat(glm::vec3(-sqrt(2.0 / 9.0), sqrt(2.0 / 3.0), -1.0 / 3.0), glm::vec3(0, 1, 1)),
			VertexFormat(glm::vec3(-sqrt(2.0 / 9.0), -sqrt(2.0 / 3.0), -1.0 / 3.0), glm::vec3(0, 0, 1)),
		...
		CreateMesh("tetrahedron_A", vertices, indices);
	}
	...
	// opposing vertex orientations.
	{
		...
		CreateMesh("square_A", vertices, indices);
	}

	{
		vector<VertexFormat> vertices
		{
			VertexFormat(glm::vec3(sqrt(8.0 / 9.0), 0,  -1.0 / 3.0), glm::vec3(0, 0, 1)),
			VertexFormat(glm::vec3(-sqrt(2.0 / 9.0), sqrt(2.0 / 3.0), -1.0 / 3.0), glm::vec3(0, 0, 1)),
			VertexFormat(glm::vec3(-sqrt(2.0 / 9.0), -sqrt(2.0 / 3.0), -1.0 / 3.0), glm::vec3(0, 0, 1))
		};
		...
		CreateMesh("trig_1", vertices, indices);
	}

	{
		...
		CreateMesh("trig_2", vertices, indices);
	}

	{
		vector<VertexFormat> vertices
		{
			VertexFormat(glm::vec3(sqrt(8.0 / 9.0), 0,  -1.0 / 3.0),glm::vec3(1, 0, 0)),
			VertexFormat(glm::vec3(-sqrt(2.0 / 9.0), sqrt(2.0 / 3.0), -1.0 / 3.0), glm::vec3(1, 0, 0)),
		...
		CreateMesh("trig_3", vertices, indices);
	}

	{
		vector<VertexFormat> vertices
		{
			VertexFormat(glm::vec3(sqrt(8.0 / 9.0), 0,  -1.0 / 3.0), glm::vec3(1, 0, 1)),
			VertexFormat(glm::vec3(-sqrt(2.0 / 9.0), -sqrt(2.0 / 3.0), -1.0 / 3.0), glm::vec3(1, 0, 1)),
			VertexFormat(glm::vec3(0, 0,  1), glm::vec3(1, 0, 1)),
		};
		...
		CreateMesh("trig_4", vertices, indices);
	}
...
void Lab2::Update(float deltaTimeSeconds)
{
	...
	RenderMesh(meshes["cube_B"], shaders["VertexColor"], glm::vec3(1.0f, 0.5f, 1.0f), glm::vec3(0.5f));
	glEnable(GL_CULL_FACE);

	// TODO(student): Draw the tetrahedron
	RenderMesh(meshes["tetrahedron_A"], shaders["VertexColor"], glm::vec3(3.0f, 0.5f, 1.0f), glm::vec3(0.5f));

	// TODO(student): Draw the square
	RenderMesh(meshes["square_A"], shaders["VertexColor"], glm::vec3(-3.0f, 0.5f, 1.0f), glm::vec3(0.5f));

	// TODO(student): Draw the tetrahedronv  with color
	RenderMesh(meshes["trig_1"], shaders["VertexColor"], glm::vec3(5.0f, 0.5f, 2.0f), glm::vec3(0.5f));
	RenderMesh(meshes["trig_2"], shaders["VertexColor"], glm::vec3(5.0f, 0.5f, 2.0f), glm::vec3(0.5f));
	RenderMesh(meshes["trig_3"], shaders["VertexColor"], glm::vec3(5.0f, 0.5f, 2.0f), glm::vec3(0.5f));
	RenderMesh(meshes["trig_4"], shaders["VertexColor"], glm::vec3(5.0f, 0.5f, 2.0f), glm::vec3(0.5f));
