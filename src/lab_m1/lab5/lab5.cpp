...
void Lab5::Update(float deltaTimeSeconds) {
	...
	{
		glm::mat4 modelMatrix = glm::mat4(1);
		modelMatrix = glm::translate(modelMatrix, glm::vec3(-4, 0.3f, 0));
		RenderMesh(meshes["box"], shaders["VertexNormal"], modelMatrix);
	}

	{
		glm::mat4 modelMatrix = glm::mat4(1);
		modelMatrix = glm::translate(modelMatrix, glm::vec3(-4, 0.3f, 3));
	...
	if (cameraType)
		...
		projectionMatrix = glm::ortho(-ortx, ortx, -orty, orty, 1.0f, 100.0f);
}
...
void Lab5::OnInputUpdate(float deltaTime, int mods) {
	...
	if (window->KeyHold(GLFW_KEY_Z) && cameraType) {
		fov -= deltaTime * 4;
	}

	if (window->KeyHold(GLFW_KEY_X) && cameraType) {
		fov += deltaTime * 4;
	}

	if (window->KeyHold(GLFW_KEY_C) && !cameraType) {
		ortx -= deltaTime * (16.0f / 9.0f);
		orty -= deltaTime;
	}

	if (window->KeyHold(GLFW_KEY_V) && !cameraType) {
		ortx += deltaTime  * (16.0f / 9.0f);
		orty += deltaTime;
	}
